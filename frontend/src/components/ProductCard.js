...
export default function ProductCard({
  ...
  return (
    <article
      className={`card group relative overflow-hidden rounded-2xl transition-shadow hover:shadow-lg ${className}`}
    >
      ...
      <div className="space-y-2 p-4">
        ...
        <div className="flex items-end justify-between pt-1">
          <p className="text-xl font-bold text-emerald-600">₹{price.toFixed(2)}</p>
          {showScore && (
        ...
        <a
          ...
          className="relative z-20 inline-flex w-full items-center justify-center rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-emerald-600"
        >
          Buy on Amazon
        </a>
      </div>
